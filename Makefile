...
.PHONY: help install-gpu install clean build test test-rust test-all
...
test-rust:	## Run Rust unit tests in the shipping profile
	@test -x $(PYTHON) || { echo "No venv found — run 'make install' first"; exit 1; }
	$(TORCH_ENV) cargo test --release

test-all: test-rust	## Run all Python and Rust tests, including sharding
	@test -x $(PYTEST) || { echo "No venv found — run 'make install' first"; exit 1; }
